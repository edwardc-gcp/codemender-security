...
CodeMender relies on a workspace configuration file located at `${PROJECT_ROOT}/.codemender/config.yaml`.
...
# Default Model: "gemini-3.8-flash" (Default, fast & accurate) or "gemini-3.1-pro-preview" (Deep reasoning)
...
build:
  command: "npm test"  # Alternatives: "pytest", "go test ./...", "cargo test", "make test"
...
| :--- | :--- | :--- | :--- |
| `model` | string | `gemini-3.8-flash` | The backend reasoning model. Supported: `gemini-3.8-flash` (default), `gemini-3.7-flash`, `gemini-3.1-pro-preview`. |
| `scan.incremental` | boolean | `true` | When `true`, caches scan state in `state.db` to only analyze modified AST subtrees on subsequent runs. |
